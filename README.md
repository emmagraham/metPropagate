...
1. METABOLOMIC_PROCESSING_PIPELINE
    ...
    The sample metabolomics files provided here are the output of an XCMS analysis comparing a single patient to a group of controls. For your interest, the XCMS code used to generate these files is commented out in the first section of the preprocessing.R file. 
...
3. integration
    ...
    wes_files contains candidate gene information (manually placed)
        - sampleA_candidate_genes.csv is an example input file from Exomiser
    LPA_output contains the post-propagation node scores (automatically generated)
...
## Quick start
 ...
 System requirements: R (at least v3.5), Python3. 
 R packages: tidyverse, CAMERA, stringr, reshape2
 Python packages: numpy, scikit-learn, sys, arg-parse, scipy.parse
 
 Zipped files that need to be unzipped before starting: 
 **integration/graph_files/STRING_graph_file_v11_functional_entire_db.txt.zip**
 **METABOLOMIC_PROCESSING_PIPELINE/HMDB_db_ver4.csv.zip**
